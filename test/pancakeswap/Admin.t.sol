...
contract PancakeSwapHookAdminTest is PancakeSwapHookBaseTest {
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_admin_updateQuoteSigner(address newSigner) public {
    vm.startPrank(admin);
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_others_updateQuoteSigner(uint256 actorIndex, address newSigner) public {
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_admin_updateEGRecipient(address newEGRecipient) public {
    vm.startPrank(admin);
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_others_updateEGRecipient(uint256 actorIndex, address newEGRecipient)
    public
  {
    address actor = actors[bound(actorIndex, 0, actors.length - 1)];
    vm.assume(actor != admin);
  ...
  function test_pancakeswap_admin_pause() public {
    vm.prank(admin);
    Management(address(hook)).pause();
  }

  function test_pancakeswap_guardian_pause() public {
    vm.prank(guardian);
    Management(address(hook)).pause();
  }

  function test_pancakeswap_unpause() public {
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_admin_or_operator_claimProtocolEGs(
    uint256 mintAmount0,
    uint256 mintAmount1,
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_admin_or_operator_claimProtocolEGs_native(
    uint256 mintAmount,
    uint256 claimAmount
  ) public {
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_others_claimProtocolEGs(
    uint256 actorIndex,
    uint256 claimAmount0,
    uint256 claimAmount1
  ) public {
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_admin_or_rescuer_rescueEGs(
    uint256 mintAmount0,
    uint256 mintAmount1,
  ...
  /// forge-config: default.fuzz.runs = 20
  function test_pancakeswap_others_rescueEGs(
    uint256 actorIndex,
    uint256 rescueAmount0,
    uint256 rescueAmount1
  ) public {
  ...
  function lockAcquired(bytes calldata data) public returns (bytes memory) {
    ...
    if (mintAmount1 == type(uint256).max) {
      vault.mint(address(hook), Currency.wrap(address(0)), mintAmount0);

      vault.sync(Currency.wrap(address(0)));
      ...
      currency0.transfer(address(vault), mintAmount0);
      vault.settle();

      vault.sync(currency1);
      currency1.transfer(address(vault), mintAmount1);
      vault.settle();
    }
  }
}
